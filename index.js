...
var commands = [["=help","Lists commands"],["=skills <ign> <profile>", "Shows the skills of a player in a profile"],["=profiles <ign>", "Lists the profiles of a player"],["=bazaar <margin/profit/smart>", "Shows best products to flip in the bazaar"],["=test", "Test command run at your own risk"]]
...
function fetchProducts() {
  ...
  for (var key in json["products"]) {
    ...
    if (status["buy_summary"].length == 0) {
      continue;
    }
    ...
    logger.write(product.toString() + ", mult: " + multiplier + "\n")
...
function bazaar(msg) {
  ...
  if (args.length == 1){
    ...
    .setColor(0xff0000)
    .setDescription("Try: =bazaar <margin/profit/smart>");
  ...
  for (var i = 0; i < 6; i ++) {
    embed.addField(products[i][0] + " (" + Math.round(products[i][10]*10)/10 + ")" ,"Buy Price: " + toKM(products[i][1]) + "\nSell Price: " + toKM(products[i][2]) + "\nBuy Volume: " + toKM(products[i][5]) + "\nSell Volume: " + toKM(products[i][6]) + "\nInstant Buys: " + toKM(products[i][8]) + "\nInstant Sells: " + toKM(products[i][9]) + "\nMargin: " + toKM(products[i][3]) + " (" + products[i][4] + "%)",true)
  }
